...
public interface CommentDAOQuery {
    ...
    public final static String GET_COMMENT_BY_ID = "select hex(id) as id, hex(userid) as userid, hex(themeid) as themeid, answer, " +
            "last_modified, creation_timestamp from comments where id=unhex(?)";
    public final static String GET_COMMENTS_BY_THEME_ID = "select hex(id) as id, hex(userid) as userid, hex(themeid) as themeid, answer, " +
